...
<%
	...
	if (request.getParameter("companyid") != null) companyid = HTMLize(request.getParameter("companyid").trim());
	String productid = request.getAttribute("productid") == null ? "" : HTMLize((String) request.getAttribute("productid"));
	if (request.getParameter("productid") != null) productid = HTMLize(request.getParameter("productid").trim());

	String productname = request.getParameter("productname") == null ? "" : HTMLize(request.getParameter("productname").trim());
	String version = request.getParameter("version") == null ? "" : HTMLize(request.getParameter("version").trim());
	String announcementdate = request.getParameter("announcementdate") == null ? "" : HTMLize(request.getParameter("announcementdate").trim());
	String shipdate = request.getParameter("shipdate") == null ? "" : HTMLize(request.getParameter("shipdate").trim());
	String url = request.getParameter("url") == null ? "" : HTMLize(request.getParameter("url").trim());
	// get rid of extra "http://" in URL if it's there
	if (url.startsWith("http://")) {
		url = url.substring(7);
	}
	String nda = request.getParameter("nda") == null ? "" : HTMLize(request.getParameter("nda").trim());
	String targetaudience = request.getParameter("targetaudience") == null ? "" : HTMLize(request.getParameter("targetaudience").trim());
	String description = request.getParameter("description") == null ? "" : HTMLize(request.getParameter("description").trim());
	String businessproblem = request.getParameter("businessproblem") == null ? "" : HTMLize(request.getParameter("businessproblem").trim());
	String competitors = request.getParameter("competitors") == null ? "" : HTMLize(request.getParameter("competitors").trim());
	String keyfeatures = request.getParameter("keyfeatures") == null ? "" : HTMLize(request.getParameter("keyfeatures").trim());
	String price = request.getParameter("price") == null ? "" : HTMLize(request.getParameter("price").trim());
	String customerreferences = request.getParameter("customerreferences") == null ? "" : HTMLize(request.getParameter("customerreferences").trim());
	...
	if (submit.equals("Save entry")) {
		...
		if (productid.equals("")) {
			...
			String sMessageText = 
				...
				"\n" +
				"Company Name: " + SMTPize(companyname) + "\n" +
				"Product or Service Name: " + SMTPize(productname) + "\n" +
				"Version: " + SMTPize(version) + "\n" +
				"Announcement Date: " + SMTPize(announcementdate) + "\n" +
				"Scheduled Ship Date: " + SMTPize(shipdate) + "\n";
				;
			if (url.equals("")) {
			...
			sMessageText += 
				"NDA or Embargo Required? " + SMTPize(nda) + "\n" +
				"\n" +
				"Target audience for product or service:\n" +
				"\n" +
				SMTPize(targetaudience) + "\n" +
				"\n" +
