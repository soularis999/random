...
<%
	...
	if (submit.equals("Submit data") || sMissingInput.equals("true")) {
		...
		password2 = request.getParameter("password2") == null ? "" : request.getParameter("password2").trim();
		...
		userphone = request.getParameter("userphone") == null ? "" : request.getParameter("userphone").trim();
		email = request.getParameter("email") == null ? "" : request.getParameter("email").trim();
		notifynextyear = request.getParameter("notifynextyear") == null ? "" : request.getParameter("notifynextyear").trim();
		...
		if (!sMissingInput.equals("true")) {
			...
			if (recordfound) {
				...
				if (!bLoggedIn) {
					request.setAttribute ("msg", "There is already a user with that e-mail address. Perhaps you already have an account? If so, we can <a href=\"accountdetailsmail.jsp\">e-mail</a> you your account details.");
					MissingInput = true;
				}
				else {
...
<table width="610">
	<tr>
		<td>
			...
			<form action="accountcreateedit.jsp">
